...
def calculate_ratios(ticker):
    ...
    try:
        ...
        with open(f"{base}_info.json") as f: info = json.load(f)
    ...
    if revenue is not None and len(income.columns) >= 2:
        ...
        ratios["revenue_growth"] = round((revenue[col]-revenue[col_prev])/abs(revenue[col_prev]), 4) if revenue[col_prev] else None

    return ratios

def main():
    WATCHLIST = load_watchlist()
    print(f"Calculating ratios for {len(WATCHLIST)} tickers: {WATCHLIST}")
    results = []
    for ticker in WATCHLIST:
        print(f"  {ticker}...")
        r = calculate_ratios(ticker)
        if r: results.append(r)
    df = pd.DataFrame(results)
    df.to_csv("data/calculated_ratios.csv", index=False)
    print(f"Done! Saved {len(results)} tickers to data/calculated_ratios.csv")
